...
const SALES_CHANNEL_ID = [process.env.NEXE_PUBLICE_SALES_CHANNEL_ID]
...
const sales_channel = process.env.NEXE_PUBLICE_SALES_CHANNEL_ID || ""
...
export async function getProductByHandle(
  ...
  const { products } = await medusaRequest("GET", "/products", {
    query: {
      handle,
      expand: "categories,images,options,variants,variants.options",
    },
  })
    .then((res) => res.body)
    .catch((err) => {
