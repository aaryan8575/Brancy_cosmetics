...
const AccountBtn = () => {
  ...
  if (customer && !retrievingCustomer) {
      ...
      <Menu as="div" className="relative inline-block text-left">
        <div>
          <Menu.Button>
            ...
            <span className="absolute bottom-0 right-0 rounded-full bg-[#ff6565] w-full aspect-square text-[12px] m-0 flex items-center justify-center translate-x-1/2">
    ...
    return (
      <Link href={"/login"} scroll={false} className="text-black relative">
        <UserIcon className="h-5" />
        <span className="absolute bottom-0 right-0 rounded-full text-black bg-white w-full aspect-square text-[12px] m-0 flex items-center justify-center translate-x-1/2 translate-y-1/3">
